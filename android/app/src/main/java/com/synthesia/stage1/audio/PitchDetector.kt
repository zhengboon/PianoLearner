...
class PitchDetector(
    ...
    private val cutoff: Float = 0.97f,         // canonical DEFAULT_CUTOFF (was 0.93 in prior impl)
    private val smallCutoff: Float = 0.5f,     // canonical SMALL_CUTOFF — skip parabolic interp below this
    ...
    fun detect(frame: ShortArray): Float? {
        ...
        // Tartini peak picker: highest local maximum between adjacent positive zero crossings.
        // We filter peaks by tau >= minTau after the fact — cleaner than mutating `pos` inside
        // the picker (which could leap us past a negative zone if minTau is large).
        val maxPositions = peakPicking(nsdf).filter { it >= minTau }
        if (maxPositions.isEmpty()) return null

        // For each peak, parabolic interp only if value > smallCutoff. Track highest amp seen.
        ...
        // First period whose amp >= cutoff * highest — picks the fundamental over harmonics.
    ...
    // Port of TarsosDSP McLeodPitchMethod.peakPicking — scans positive segments
    // bounded by negative zero crossings, keeps the highest local maximum in each segment.
    // No min-tau filter here; caller filters peaks post-process.
    private fun peakPicking(nsdf: FloatArray): List<Int> {
        ...
        // Skip the initial positive lobe (tau~0 always has nsdf ~ 1).
        while (pos < (n - 1) / 3 && nsdf[pos] > 0f) pos++
        // Skip the first negative region.
